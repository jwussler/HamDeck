...
public class ApiServer : IDisposable
{
    ...
    // BUG FIX: _freqBuffer is accessed from multiple concurrent Task.Run handlers.
    // All reads and writes must be serialized through this lock.
    ...
    private void HandleRequest(HttpListenerContext ctx)
    {
        ...
        // BUG FIX: Route() returns null for unknown routes. Set HTTP 404 instead of
        // silently returning 200 with { "status": "error" }. Callers (Stream Deck,
        // monitoring tools) can now distinguish a routing miss from a successful response.
        var (statusCode, result) = Route(path);
        WriteJson(resp, result, statusCode);
    }

    private (int status, object result) Route(string path)
    {
        path = path.TrimEnd('/');

        // ===== INDEX / HEALTH =====
        if (path == "" || path == "/")
            return OK(new { service = "HamDeck API (C#)", port = _config.APIPort });
        if (path == "/api/test")
            return OK(new { ok = true, message = "API is working" });
        if (path == "/api/health")
            return OK(new { status = "ok", service = "HamDeck API (C#)", version = "2.0",
                port = _config.APIPort, rig_connected = _radio.Connected,
                amp_tuning = _amp.IsActive, tgxl_tuning = _tgxl.IsActive,
                freq_buffer = GetFreqBuffer() });

        // ===== STATUS =====
        if (path == "/api/status")
        {
            if (!_radio.Connected)
                return OK(new { connected = false, amp_tuning = _amp.IsActive,
                    tgxl_tuning = _tgxl.IsActive, freq_buffer = GetFreqBuffer() });
            return OK(new { connected = true, freq = _radio.GetFreq(), mode = _radio.GetMode(),
                vfo = _radio.GetVFO(), power = _radio.GetPower(), tx = _radio.GetTXStatus(),
                split = _radio.GetSplit(), ant = _radio.GetAntenna(), rxant = _radio.GetRxAntenna(),
                amp_tuning = _amp.IsActive, tgxl_tuning = _tgxl.IsActive,
                freq_buffer = GetFreqBuffer() });
        }
        ...
        {
            var m = path["/api/mode/".Length..].ToUpper();
            _radio.SetMode(m);
            return OK("mode", m);
        }
        ...
        foreach (var (key, freq) in BandHelper.BandFrequencies)
        {
            if (path == $"/api/band/{key}")
            {
                ...
                return OK(new { status = "ok", band = key, freq, mode });
            }
        }
        ...
        // ===== POWER =====
        if (path == "/api/power/qrp")  { _radio.SetPower(5);   return OK(new { status = "ok", power = "qrp",  watts = 5 }); }
        if (path == "/api/power/low")  { _radio.SetPower(25);  return OK(new { status = "ok", power = "low",  watts = 25 }); }
        if (path == "/api/power/mid")  { _radio.SetPower(50);  return OK(new { status = "ok", power = "mid",  watts = 50 }); }
        if (path == "/api/power/high") { _radio.SetPower(100); return OK(new { status = "ok", power = "high", watts = 100 }); }
        if (path == "/api/power/max")  { _radio.SetPower(200); return OK(new { status = "ok", power = "max",  watts = 200 }); }
        if (path.StartsWith("/api/power/set/") && int.TryParse(path["/api/power/set/".Length..], out var pw))
            { _radio.SetPower(pw); return OK("power", pw); }

        // ===== FREQUENCY =====
        if (path == "/api/freq")
            return OK(new { freq = _radio.GetFreq() });
        if (path.StartsWith("/api/freq/set/") && long.TryParse(path["/api/freq/set/".Length..], out var fq))
        {
            ...
            return OK(new { status = "ok", freq = fq, mode });
        }

        // ===== FREQ KEYPAD BUFFER — all operations are lock-protected =====
        if (path.StartsWith("/api/freq/digit/") && path.Length > "/api/freq/digit/".Length)
        {
            var d = path["/api/freq/digit/".Length..];
            string buf;
            lock (_bufLock) { _freqBuffer += d; buf = _freqBuffer; }
            return OK("buffer", buf);
        }
        if (path == "/api/freq/clear")
        {
            lock (_bufLock) { _freqBuffer = ""; }
            return OK("buffer", "");
        }
        if (path == "/api/freq/backspace")
        {
            string buf;
            lock (_bufLock)
            {
                if (_freqBuffer.Length > 0) _freqBuffer = _freqBuffer[..^1];
                buf = _freqBuffer;
            }
            return OK("buffer", buf);
        }
        if (path == "/api/freq/get")
        {
            string buf;
            lock (_bufLock) { buf = _freqBuffer; }
            return OK(new { status = "ok", buffer = buf, length = buf.Length });
        }
        if (path == "/api/freq/send")
            return SendFreqBuffer();
        ...
        if (path.StartsWith("/api/step/"))
        {
            ...
            if (parts.Length >= 5 && long.TryParse(parts[3], out var hz))
            {
                ...
                _radio.StepFreq(hz);
                return OK(new { status = "ok", step = Math.Abs(hz), direction = parts[4] });
            }
        }

        // ===== TUNERS =====
        if (path == "/api/tune")                    { _radio.StartTune(); return OK("action", "tuning"); }
        if (path is "/api/tune/tgxl" or "/api/tgxl/tune") return OK(_tgxl.Tune());
        if (path == "/api/tune/tgxl/status")        return OK(new { status = "ok", tuning = _tgxl.IsActive });
        if (path is "/api/tune/amp"  or "/api/amp/tune")   return OK(_amp.Tune(30, 0.15));
        if (path == "/api/tune/amp/status")         return OK(new { status = "ok", tuning = _amp.IsActive });
        if (path.StartsWith("/api/tune/amp/"))
        {
            ...
            if (parts.Length >= 5 && parts[4] != "status" &&
                double.TryParse(parts[4], System.Globalization.NumberStyles.Float,
                    System.Globalization.CultureInfo.InvariantCulture, out var p)) power = p;
            if (parts.Length >= 6 && int.TryParse(parts[5], out var d)) dur = d;
            return OK(_amp.Tune(dur, power));
        }

        // ===== RECORDING =====
        if (path == "/api/record/status")  return OK(_recorder.GetStatus());
        if (path == "/api/record/start")   { _recorder.Start(); return OK("recording", true); }
        if (path == "/api/record/stop")    { var fn = _recorder.Stop(); return OK(new { status = "ok", filename = fn }); }
        if (path is "/api/record/toggle" or "/api/record/toggle/stereo")
        {
            if (_recorder.IsRecording) { var fn = _recorder.Stop(); return OK(new { status = "ok", action = "stopped", filename = fn }); }
            _recorder.Start(); return OK(new { status = "ok", action = "started" });
        }
        if (path == "/api/record/replay") { var fn = _recorder.SaveReplay(); return OK(new { status = "ok", filename = fn }); }

        // ===== VOLUME =====
        if (path == "/api/volume/get")
            { var v = _radio.GetAFGain(); return OK(new { status = "ok", volume = v * 100 / 255, raw = v }); }
        ...
        if (path == "/api/mute/toggle")
            { if (_radio.GetAFGain() > 0) { _radio.SetAFGain(0); return OK("mute", 1); } _radio.SetAFGain(128); return OK("mute", 0); }

        // ===== FILTERS =====
        if (path == "/api/toggle/nb")            { var c = _radio.GetNB();    _radio.SetNB(!c);    return OK("nb", !c); }
        if (path == "/api/toggle/dnr" || path == "/api/toggle/nr")
                                                 { var c = _radio.GetNR();    _radio.SetNR(!c);    return OK("nr", !c); }
        ...
        // ===== CW =====
        if (path == "/api/cw-speed/get")
            return OK(new { status = "ok", wpm = _radio.GetCWSpeed() });
        if (path == "/api/cw-speed/up")
            { var w = _radio.GetCWSpeed(); _radio.SetCWSpeed(w + 2); return OK("wpm", w + 2); }
        if (path == "/api/cw-speed/down")
            { var w = _radio.GetCWSpeed(); _radio.SetCWSpeed(w - 2); return OK("wpm", w - 2); }
        ...
        // ===== METERS =====
        if (path == "/api/meters")
            return OK(new { status = "ok", s_meter = _radio.GetSMeter(), swr = _radio.GetSWR(),
                alc = _radio.GetALC(), power = _radio.GetPowerMeter() });
        ...
        // ===== WIDTH =====
        if (path == "/api/width/narrow") { _radio.SetWidth(6); return OK(new { status = "ok", width = "narrow", hz = 1800 }); }
        if (path == "/api/width/medium") { _radio.SetWidth(10); return OK(new { status = "ok", width = "medium", hz = 2400 }); }
        if (path == "/api/width/wide")   { _radio.SetWidth(14); return OK(new { status = "ok", width = "wide",   hz = 3000 }); }
        ...
        // ===== TX ANTENNA (ANT1/ANT2/ANT3 ports) =====
        ...
        if (path == "/api/ant/get")    return OK(new { status = "ok", ant = _radio.GetAntenna() });

        // ===== RX ANTENNA INPUT (dedicated RX port) =====
        ...
        if (path == "/api/ant/rx/get")    return OK(new { status = "ok", rxant = _radio.GetRxAntenna() });

        // ===== KMTRONIC RX ANTENNA RELAY =====
        if (path.StartsWith("/api/rxant/") && _kmtronic != null)
        {
            var seg = path["/api/rxant/".Length..];
            if (seg == "get") return OK(new { status = "ok", rxant = _kmtronic.ActiveAntenna });
            if (seg == "cycle")
            {
                var next = (_kmtronic.ActiveAntenna % 4) + 1;
                _kmtronic.SetAntenna(next);
                return OK("rxant", next);
            }
        ...
        return (404, (object)new { status = "error", message = "Unknown route", path });
    }

    // ── _freqBuffer helpers (all access must go through these or lock(_bufLock)) ──

    private string GetFreqBuffer()
    {
        lock (_bufLock) { return _freqBuffer; }
    }

    private (int status, object result) SendFreqBuffer()
    {
        string buf;
        lock (_bufLock) { buf = _freqBuffer; }

        if (string.IsNullOrEmpty(buf))
            return (400, (object)new { status = "error", message = "Buffer is empty" });

        long freqHz;
        if (buf.Length <= 3)
        {
            long.TryParse(buf, out var mhz);
            freqHz = mhz * 1_000_000;
        }
        else
        {
            long.TryParse(buf[..^3], out var mhz);
            long.TryParse(buf[^3..], out var khz);
        ...
        return OK(new { status = "ok", freq_hz = freqHz, mode, cleared = true });
    }

    // ── Helpers ──

    private static (int, object) OK() => (200, (object)new { status = "ok" });
    private static (int, object) OK(string key, object val) =>
        (200, (object)new Dictionary<string, object> { ["status"] = "ok", [key] = val });
    private static (int, object) OK(object val) => (200, val);
