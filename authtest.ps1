...
function Chk($route, $exp) {
    try { $c = (Invoke-WebRequest -Uri ($BASE+$route) -UseBasicParsing -SkipHttpErrorCheck -EA SilentlyContinue).StatusCode }
...
foreach($r in @("admin/users","admin/sessions","admin/radio","admin/presets","admin/lockdown/status","admin/flexknob/buttons")){
    try { $c = (Invoke-WebRequest -Uri ($BASE+"/"+$r) -UseBasicParsing -SkipHttpErrorCheck -EA SilentlyContinue).StatusCode }
    catch { try { $c = $_.Exception.Response.StatusCode.value__ } catch { $c = 0 } }
